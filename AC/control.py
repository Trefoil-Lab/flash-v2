...
from control.supply import ACSupply
from util.signals import Params, ControlSignals, SampleSignals, GuiSignals

RAMP_INTERVAL_S = 0.200
SAVE_INTERVAL_S = 10
CSV_HEADER = 'Timestamp,Time,Volts(rms),Amps(rms),|S|(VA),real power(W),Q(VAr),E,J,P,T'
...
class ControlRunner(QRunnable):
    def __init__(
        ...
        self.supply = ACSupply(self.addr)
...
class SampleRunner(Thread):
    ...
    def run(self):
        ...
        def sample(sc : sched.scheduler, target_time : float):
            ...
            with self.supply_lock:
                data = [0, # time
                        self.supply.measVAC(), # voltage 
                        self.supply.measIAC(), # current
                        self.supply.measP_apparent(), # apparent power
                        self.supply.measP_real(), # real power
                        self.supply.measP_reactive(), # reactive power
            ...
            queue_data = (
                ...
                data[1], # voltage
                data[2], # current
                data[3], # apparent power
                data[4], # real power
                data[5], # reactive power
